...
void SpalartAllmaras<BasicTurbulenceModel>::correct()
{
    ...
    (
        CUSTOM_SCALAR_1*fvm::ddt(alpha, rho, nuTilda_)
      + CUSTOM_SCALAR_2*fvm::div(alphaRhoPhi, nuTilda_)
      - CUSTOM_SCALAR_3*fvm::laplacian(alpha*rho*DnuTildaEff(), nuTilda_) 
      - CUSTOM_SCALAR_4*Cb2_/sigmaNut_*alpha*rho*magSqr(fvc::grad(nuTilda_))
     ==
        CUSTOM_SCALAR_5*Cb1_*alpha*rho*Stilda*nuTilda_
      - CUSTOM_SCALAR_6*fvm::Sp(Cw1_*alpha*rho*fw(Stilda)*nuTilda_/sqr(y_), nuTilda_)
      + CUSTOM_SCALAR_7*fvOptions(alpha, rho, nuTilda_)
    );
